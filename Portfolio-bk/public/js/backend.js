...
function onNaviClick() {
  $(".dash-border").css({"z-index": 997, "background-color": "#7295c0"});
...
$.get('../json/backend.json', onBackEndLoad);

function onBackEndLoad(r){
  pfLast = r.backend.length -1;
  var html;
  for (var i in r.backend) {
    html  = '<div class="desc-wrapper">';
    html += '<div class="title-wrap">';
    html += '<h3 class="site-title">'+r.backend[i].title+'</h3>';
    html += '<div class="desc-wrap">';
    html += '<p> 100% 개인 작업</p>';
    html += '<p>제작 기간 :'+r.backend[i].during+'</p>';
    html += '</div>';
    html += '</div>';
    html += '<div class="sub-wrap">';
    html += '<p class="renewal">'+r.backend[i].renewal+'</p>';
    html += '<p class="used">';
    html += '<span>#Used</span> &nbsp'+r.backend[i].used+'';
    html += '</p>';
    html += '<p class="skill">'+r.backend[i].skill+'</p>';
    ...
    html += '<div class="color-box cb1" style="background-color:'+r.backend[i].colorName1+';"></div>';
    ...
    html += '<p class="font1" style="font-family: '+r.backend[i].font1+'">'+r.backend[i].font1+'</p>';
    html += '<p class="font2" style="font-family: '+r.backend[i].font2+'">'+r.backend[i].font2+'</p>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
    html += '</div>';
    ...
    html += '</video>';
    html += '</div>';
    html += '<a href="'+r.backend[i].siteHref+'" target="_blank" class="bt-view">View Site</a>';
    html += '</div>';

	  pfRt.push($(html));
  }
  for (var i=0; i<=r.backend.length-1; i++) $(".pager-wrap").append('<i class="pager fa-circle"></i> ');
	$(".pager").click(onPager);
  pfInit();
}
