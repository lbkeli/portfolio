...
function onSave() { // f = form
	...
	if($writer.val().trim() == "") { // trim : 앞 뒤 공백 잘라냄
		alert('작성자란이 비어있습니다.');
		$writer.focus();
		return false;
	}
	if($comment.val().trim() == "") { // trim : 앞 뒤 공백 잘라냄
		alert('한줄 내용이 비어있습니다.');
		$comment.focus();
		return false;
	}
	...
	$("form[name='gbookForm']")[0].reset(); // 작성란 비우기
	$.post('/gbook/api/save', sendData, onResponse); // AJAX 통신
	// post방식 : 통신할 주소, 전달할 데이터 (보낼 데이터 없으면 안써도 됨), 송신해서 나온 결과 (전달받은 콜백)
}
