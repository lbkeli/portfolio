...
function onDownload() {
	// gallery/download/3?seq= | gallery/download/3?seq=2
...
function onRev(id, savefile, savefile2) {
	if(confirm("정말로 삭제하시겠습니까?")) {
		location.href = '/gallery/rev/'+id+'?savefile='+savefile+'&savefile2='+savefile2; // get 방식 query
	}
}

function onImgRev(id, n, file) {
	if(confirm("이미지를 삭제하시겠습니까?")) {
		$.get('/gallery/api-img/'+id, {n: n, file: file}, function(r){ // params, query // get - query | post - body
			if(r.code == 200) {
				if(n == '') {
					$(".wr-wrap .img1").remove();
				...
				console.log(r.error);
				alert('회원만 사용 가능합니다. 로그인 후 이용하세요.');
			}
		});
	}
}
